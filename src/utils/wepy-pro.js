    ...
    return new Promise((resolve, reject) => {
      ...
      wepy.request({
        ...
        success(res) {
          ...
          if (statusCode === 200) {
            if (errorCode == 0) {
              if (options.noOutData) {  //分页字段noOutData  true可分页   false或者不填则为不分页 （根据后台数据结构取不同数据）
                resolve(res.data)
              } else {
                resolve(res.data.data)
              }
            } else {
              console.log("到这里errMsg数据结构有问题")
              console.log(errMsg)
              miniPro.showToast(errMsg)
              reject(data)
            }
          } else if (statusCode == 401) { //token失效重新登录
            wepy.setStorageSync('isGettingToken', false) //isGettingToken 为false走登录流程 如果为true则不走
