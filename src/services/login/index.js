...
import { WECHAT_APP_NAME, WECHAT_AUTH_BASE, EVENT_QUEUE_NAME } from '../../common/js/config'
// import { request } from '../request'
...
const loginFetch = (data) => packagePromise((resolve, reject) => {
  ...
  authProcess(() => {
    request({
      ...
      method: 'POST',
      // header: {
      //   'Authorization': `Basic ${WECHAT_AUTH_BASE}`  //先注释
      // },
      data: {
        grant_type: 'wechat',
        sms_verify: true,
...
const wechatLogin = (data) => packagePromise((resolve, reject) => {
  ...
  let qrSi = wepy.$instance.globalData.qrSi
  let qrSk = wepy.$instance.globalData.qrSk
  let room_id = wepy.$instance.globalData.room_id
  request({
    url: apiWechatLogin(),
    method: 'POST',
    // header: {
